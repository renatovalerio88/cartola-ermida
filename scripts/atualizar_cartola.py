...
def carregar_rodada_atual():
    try:
        with ARQUIVO_RODADA_ATUAL.open("r", encoding="utf-8") as arquivo:
            dados = json.load(arquivo)
...
def mapa_rodada_atual(rodada):
    dados = carregar_rodada_atual()
    if int(dados.get("rodada_dados", 0) or 0) != int(rodada):
        return {}
    times = dados.get("times", [])
    if not isinstance(times, list) or len(times) != TOTAL_TIMES:
        return {}
    mapa = {}
    for item in times:
        ...
        if time_id > 0:
            mapa[time_id] = item
    return mapa if len(mapa) == TOTAL_TIMES else {}
...
# O workflow executa atualizar_rodada_atual.py antes deste script.
# Portanto, quando houver um snapshot completo da mesma rodada, ele é a
# fonte preferencial do resultado final. /time/id permanece como fallback.
mapa_atual = mapa_rodada_atual(rodada_para_salvar)
if mapa_atual:
    print("Fonte preferencial: rodada_atual_cartola.json validado.")
else:
    print("AVISO: snapshot atual indisponível; usando /time/id como fallback.")
...
for indice, (time_id, nome_time, cartoleiro) in enumerate(TIMES, start=1):
    ...
    try:
        ...
        snapshot = mapa_atual.get(time_id, {})
        pontos_snapshot = snapshot.get("pontos") if isinstance(snapshot, dict) else None
        patrimonio_snapshot = snapshot.get("patrimonio") if isinstance(snapshot, dict) else None
        ...
        registro = {
            ...
            "pontos": round(
                float(
                    pontos_snapshot
                    if pontos_snapshot is not None
                    else dados.get("pontos", 0) or 0
                ),
                2,
            ),
            "patrimonio": round(
                float(
                    patrimonio_snapshot
                    if patrimonio_snapshot is not None
                    else dados.get("patrimonio", 0) or 0
                ),
                2,
            ),
            "fonte_pontos": (
                "rodada_atual_cartola"
                if pontos_snapshot is not None
                else "api_time_id_fallback"
            ),
        }
