...
def obter_mapa_pontuados(dados):
    ...
    if not isinstance(atletas, dict):
        raise ValueError(
            "A API de pontuados não retornou um mapa de atletas."
        )

    return atletas


def obter_pontuacao_atleta(mapa_pontuados, atleta_id):
    dados = mapa_pontuados.get(str(atleta_id), {})
    ...
    return numero(
        dados.get(
            ...
            dados.get(
                "pontos",
                dados.get("pontos_num", 0),
            ),
        )
    )


def atleta_entrou_em_campo(mapa_pontuados, atleta_id):
    dados = mapa_pontuados.get(str(atleta_id), {})

    if not isinstance(dados, dict):
        return False

    return bool(dados.get("entrou_em_campo", False))
...
def calcular_parcial(
    ...
    if len(atletas) < 11:
        raise ValueError(
            f"Escalação incompleta: apenas {len(atletas)} atletas."
        )
    ...
    for atleta in atletas:
        ...
        eh_capitao = atleta_id == capitao_id

        if eh_capitao:
            pontuacao_computada = (
                pontuacao_normal * MULTIPLICADOR_CAPITAO
            )
        else:
            pontuacao_computada = pontuacao_normal

        if atleta_id in {
            inteiro(chave)
            for chave in mapa_pontuados.keys()
        ...
        detalhes.append(
            {
                ...
                "pontos": round(pontuacao_normal, 2),
...
rodada_status = inteiro(status.get("rodada_atual", 0))
mercado_status = inteiro(status.get("status_mercado", 0))
mercado_aberto = mercado_status == 1
bola_rolando_api = bool(status.get("bola_rolando", False))

if rodada_status <= 0:
    raise RuntimeError(
        "Não foi possível determinar a rodada atual do Cartola."
    )

print(f"Rodada Cartola: {rodada_status}")
print(f"Status mercado: {mercado_status}")
print(f"Mercado aberto: {mercado_aberto}")
print(f"Bola rolando informada pela API: {bola_rolando_api}")


print("\nConsultando atletas pontuados...")
dados_pontuados = buscar_json(URL_PONTUADOS)

rodada_pontuados = inteiro(
    dados_pontuados.get("rodada", 0)
)
total_atletas_pontuados = inteiro(
    dados_pontuados.get("total_atletas", 0)
)
...
if total_atletas_pontuados <= 0:
    total_atletas_pontuados = len(mapa_pontuados)
...
print(f"Rodada da API de pontuados: {rodada_pontuados}")
print(f"Atletas pontuados: {total_atletas_pontuados}")
print(f"Parciais disponíveis: {parciais_disponiveis}")
print(f"Rodada em andamento calculada: {rodada_em_andamento}")
...
if rodada_dados <= 0:
    raise RuntimeError(
        "Não foi possível determinar a rodada dos dados."
    )
...
print(
    ...
    if rodada_em_andamento
    else f"Buscando dados fechados da rodada {rodada_dados}..."
)


for indice, (time_id, nome_time, cartoleiro) in enumerate(
    TIMES,
    start=1,
):
    url = (
        f"https://api.cartola.globo.com/time/id/"
        f"{time_id}/{rodada_dados}"
    )
    ...
    try:
        ...
        if rodada_em_andamento:
            if rodada_escalacao != rodada_status:
                raise ValueError(
                    "Escalação retornada para a rodada "
                    f"{rodada_escalacao}; esperada "
                    f"{rodada_status}."
                )
            ...
            pontos = round(
                numero(dados_time.get("pontos", 0)),
                2,
            )
        ...
        if rodada_em_andamento:
            registro["capitao_id"] = inteiro(
                dados_time.get("capitao_id", 0)
            )
            registro["detalhes_parcial"] = detalhes

        novos_times.append(registro)

        print(
            f"[{indice:02d}/{TOTAL_TIMES}] "
            f"OK - {nome_time}: {pontos:.2f}"
        )
...
if erros or len(novos_times) != TOTAL_TIMES:
    ...
    raise RuntimeError(
        "Não foi possível calcular os 36 times."
    )


if rodada_em_andamento:
    observacao = (
        "Parciais calculadas pela escalação dos times "
        "e pela API oficial de atletas pontuados. "
        "O capitão recebe multiplicador de 1,5."
    )

elif mercado_aberto:
    observacao = (
        "Mercado aberto: exibindo os pontos da "
        "última rodada fechada."
    )

else:
    observacao = (
        "Mercado fechado, aguardando os primeiros "
        "atletas pontuados da rodada atual."
    )
...
if rodada_em_andamento:
    parciais = dict(saida)
    parciais["fonte"] = "parciais_cartola.json"
